    ...
    for (t=0;t<=TEND;t+=dt){
        ...
        for (i=3;i<blockH-3;i++)
            for (j=3;j<blockL-3;j++){
                ...
                // for (k=0;k<4;k++){
                //     u[i][j].param.fx[k]=u[i][j].param.fp[k]+u[i][j].param.fn[k];
                //     u[i][j].param.gy[k]=u[i][j].param.gp[k]+u[i][j].param.gn[k];
        ...
        if (id==1){
            ...
            for (i=0;i<blockH-0;i++){
                for (j=0;j<blockL-0;j++){
